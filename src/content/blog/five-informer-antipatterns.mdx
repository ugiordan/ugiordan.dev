...
<Image src={predicatesFilter} alt="Predicates sit between the informer and the work queue. The informer still LIST+WATCHes every ConfigMap cluster-wide, deserializes all of them, and stores them in memory. The predicate only filters which events reach the reconciler." />
...
<Image src={disableforPaths} alt="DisableFor only affects client.Get() and client.List() calls. Owns() and Watches() create independent informers through the controller builder, completely bypassing DisableFor." />
...
<Image src={clientGetFlow} alt="When client.Get() is called for a type not in ByObject, controller-runtime silently creates a cluster-wide informer on the first access. The informer LISTs every object of that type, caches them all, then returns the single one you asked for." />
...
<Image src={typedUnstructuredCaches} alt="Three separate caches in controller-runtime: typed, unstructured, and metadata. They don't share entries. A ConfigMap cached as a typed corev1.ConfigMap is invisible to a client.Get requesting an unstructured.Unstructured." />
...
<Image src={memoryLayout} alt="Memory layout when watch and read paths use different representations. The typed cache stores full Go structs that never serve reads, while the unstructured cache may create a second informer for the same type." />
...
<Image src={twoCodePaths} alt="Two code paths that create informers: (1) controller builder registrations via For(), Owns(), and Watches(), and (2) implicit informers from client.Get() and client.List() calls when a type isn't in ByObject." />
