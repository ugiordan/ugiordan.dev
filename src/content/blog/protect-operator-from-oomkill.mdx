...
<Figure src={informerUnfiltered} alt="Unfiltered informer cache" caption="An unfiltered informer watches every ConfigMap cluster-wide, deserializing each one into memory regardless of whether the operator needs it" number={1} />
...
<Figure src={oomkillAttackFlow} alt="OOMKill attack flow" caption="A regular user floods large ConfigMaps, the unfiltered informer caches them all, memory exceeds the pod limit, and the operator enters CrashLoopBackOff" number={2} />
...
<Figure src={fixSteps} alt="Fix steps overview" caption="Four phases of the fix: filter the cache, label your own ConfigMaps, handle the upgrade path, and propagate labels during updates" number={3} />
...
<Figure src={informerFiltered} alt="Filtered informer cache" caption="With a label selector, only operator-owned ConfigMaps enter the cache. Everything else is ignored." number={4} />
...
<Figure src={upgradePath} alt="Upgrade path flow" caption="Pre-existing ConfigMaps without the label are invisible to the filtered cache but still exist in the API server, requiring a merge patch to resolve" number={5} />
...
<Figure src={clusterTestFlow} alt="Cluster test flow" caption="Validation lifecycle: flood ConfigMaps, observe OOMKill on the unpatched operator, deploy the fix into the same hostile environment, confirm stable memory" number={6} />
